...
import { MOCK_MODELS, MOCK_CODESPACES, MOCK_VOLUMES, ADMIN_DATASET_ROWS, ADMIN_VECTOR_SEARCH_RESULTS, MOCK_VECTOR_COLLECTIONS, MOCK_EMBED_STATUS } from '../mocks.js';
...
export const VectorDbPage = () => {
  ...
  const [collections]=useState(MOCK_VECTOR_COLLECTIONS);
  ...
  return (
    <PageShell breadcrumb={['데이터','벡터 DB']}>
      <div className="grid grid-cols-3 gap-4 mb-6">
        <div className="bg-white p-5 rounded-xl border"><div className="text-3xl font-bold mb-1">{Number(MOCK_EMBED_STATUS?.vectorDb?.totalVectors ?? collections.reduce((a,c)=>a+(Number(c.vectors)||0),0)).toLocaleString()}</div><div className="text-sm text-gray-500">총 벡터 수</div></div>
      ...
      <div className="bg-white rounded-xl border overflow-hidden">
        <table className="w-full text-sm"><thead className="bg-gray-50 text-xs text-gray-500 uppercase"><tr>
          ...
          <tr key={i} className="hover:bg-gray-50"><td className="px-4 py-3 font-mono text-sm font-medium">{c.name}</td><td className="px-4 py-3 text-right">{typeof c.vectors==='number'?c.vectors.toLocaleString():c.vectors}</td><td className="px-4 py-3 text-right">{c.dim}</td><td className="px-4 py-3"><StatusBadge status={c.status}/></td><td className="px-4 py-3 text-gray-500 text-xs">{c.updated}</td></tr>
        ))}</tbody></table>
      </div>
    </PageShell>
  );
